                     ...
                     short int **smoothedim)
{
    ...
    ****************************************************************************/
    ...
    omp_set_dynamic(0);                       // dynamic adjustment is disabled, # of threads remains fixed
    omp_set_num_threads(n_thread);            // maximum # of threads
#pragma omp parallel private(c, cc, dot, sum) // delcares data to have a separate copy in mem of each thread
    {
        if (VERBOSE)
...
#pragma omp parallel private(r, rr, dot, sum) // c, cc -> r, rr
    {
        if (VERBOSE)
        ...
        resultrowptr += ncols)
    {
        for (colcount = 1, magptr = magrowptr, gxptr = gxrowptr, gyptr = gyrowptr,
            resultptr = resultrowptr;
             colcount <= ncols - 2; /* bug fix 3/29/17, RD */
