...
int main(int argc, char **argv) {
  ...
  struct stat st = {0};
  if (stat("EDGE_IMAGE", &st) == -1) {
    mkdir("EDGE_IMAGE", 0755);
  }
  if (stat("RAW_IMAGE", &st) == -1) {
    mkdir("RAW_IMAGE", 0755);
  }
  ...
  while (count < NFRAME) {
     ...
     ****************************************************************************/
    sprintf(outfilename, "EDGE_IMAGE/EDGE_%03d.pgm", count);
    if (VERBOSE)
     ...
     ****************************************************************************/
    sprintf(outfilename, "RAW_IMAGE/RAW_%03d.pgm", count);
    if (VERBOSE)
      printf("Writing the edge image in the file %s.\n", outfilename);
  ...
  while (count < NFRAME) {
    ...
    sprintf(edge_image_name, "EDGE_IMAGE/EDGE_%03d.pgm", count);

    /* Get the raw image name */
    sprintf(raw_image_name, "RAW_IMAGE/RAW_%03d.pgm", count);
