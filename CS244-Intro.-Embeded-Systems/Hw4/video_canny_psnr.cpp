...
void *cal_psnr(void *arguments) {
  ...
  for (int i = start; i < end; i++) {
    ...
    sprintf(edge_image_name, "EDGE_VIDEO/EDGE_%03d.pgm", i);
    /* Get the raw image name */
    sprintf(raw_image_name, "RAW_VIDEO/RAW_%03d.pgm", i);
...
int main(int argc, char **argv) {
  ...
  struct stat st = {0};
  if (stat("EDGE_VIDEO", &st) == -1) {
    mkdir("EDGE_VIDEO", 0755);
  }
  if (stat("RAW_VIDEO", &st) == -1) {
    mkdir("RAW_VIDEO", 0755);
  }
  ...
  while (count < NFRAME) {
     ...
     ****************************************************************************/
    sprintf(outfilename, "EDGE_VIDEO/EDGE_%03d.pgm", count);
    if (VERBOSE)
     ...
     ****************************************************************************/
    sprintf(outfilename, "RAW_VIDEO/RAW_%03d.pgm", count);
    if (VERBOSE)
      printf("Writing the edge image in the file %s.\n", outfilename);
